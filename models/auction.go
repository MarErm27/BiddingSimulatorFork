...
import (
	...
	"github.com/MarErm27/BiddingSimulator/MarErm"
...
type Auction struct {
	...
	Buyer                           string `uadmin:"required;search"`
	PublicationDate                 time.Time
	ApplicationDeadline             time.Time
	BiddingStart                    time.Time
	BiddingFinish                   time.Time
	Status                          string
	InitialPrice                    string `uadmin:"money;search;pattern:^[1-9][0-9]*([.][0-9]{2}|)*$;pattern_msg:Your input must be a number."`
	HalfPercentOfInitialPrice       string `uadmin:"money;pattern:^[1-9][0-9]*([.][0-9]{2}|)*$;pattern_msg:Your input must be a number."`
	FivePercentsOfInitialPrice      string `uadmin:"money;pattern:^[1-9][0-9]*([.][0-9]{2}|)*$;pattern_msg:Your input must be a number."`
	CurrentPrice                    string `uadmin:"money;pattern:^[1-9][0-9]*([.][0-9]{2}|)*$;pattern_msg:Your input must be a number."`
